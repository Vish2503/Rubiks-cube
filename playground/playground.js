...
function moveStart(event) {
    ...
    if (event.type === "mousedown") {
        pointer.x =
            -1 + (2 * event.offsetX) / world.renderer.domElement.clientWidth;
        pointer.y =
            1 - (2 * event.offsetY) / world.renderer.domElement.clientHeight;
...
function moveOngoing(event) {
    ...
    if (!selectedFace) {
        if (event.type === "mousemove") {
            pointer.x =
                -1 +
                (2 * event.offsetX) / world.renderer.domElement.clientWidth;
            pointer.y =
                1 -
                (2 * event.offsetY) / world.renderer.domElement.clientHeight;
        } else if (event.type === "touchmove") {
            const { top, left, width, height } =
                world.renderer.domElement.getBoundingClientRect();
            pointer.x = -1 + (2 * (event.touches[0].clientX - left)) / width;
        ...
        try {
            ...
            directionOfFaceOnRightNow.set(
                Math.abs(array[0]),
                Math.abs(array[1]),
                Math.abs(array[2])
            );
            if (
                firstFaceDirectionWhenNoSelection.equals(new Vector3(0, 0, 0))
            ) {
                firstFaceDirectionWhenNoSelection.copy(
                    directionOfFaceOnRightNow
                );
            }
            if (
                secondFaceDirectionWhenNoSelection.equals(
                    new Vector3(0, 0, 0)
                ) &&
                !firstFaceDirectionWhenNoSelection.equals(
                    directionOfFaceOnRightNow
                )
            ) {
                secondFaceDirectionWhenNoSelection.copy(
                    directionOfFaceOnRightNow
                );
            }
        } catch (error) {}
