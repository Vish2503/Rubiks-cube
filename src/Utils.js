...
function generateScramble() {
    function getRandomMove() {
        return (
            scrambleMoves[Math.floor(Math.random() * scrambleMoves.length)] +
            scrambleDirs[Math.floor(Math.random() * scrambleDirs.length)]
        );
    }
    let scramble = [];
    ...
    for (let i = 2; i < 25; i++) {
        ...
        while (
            scramble[i - 1].charAt(0) === move.charAt(0) ||
            scramble[i - 2].charAt(0) === move.charAt(0)
        ) {
            move = getRandomMove();
        }
        scramble.push(move);
...
function shrinkMoveArray(moveArray, keepNonMovesFlag = false) {
    ...
    while (true) {
        for (let i = 0; i < moveArray.length; i++) {
            ...
            if (
                moveArray[i + 1] &&
                moveArray[i].charAt(0) === moveArray[i + 1].charAt(0)
            ) {
                totalCount++;
