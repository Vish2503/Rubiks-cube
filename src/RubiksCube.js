...
class Rubikscube {
    ...
    move(move) {
        ...
        return new Promise(async (resolve) => {
            ...
            for (const i of x) {
                for (const j of y) {
                    for (const k of z) {
                        ...
                        this.pieces[i_][j_][k_].cube.attach(
                            this.pieces[a][b][c].cube
                        );
                    }
                }
            }
            ...
            await this.doMove(
                this.pieces[i_][j_][k_].cube,
                dir,
                axis,
                this.moveAnimationSpeed
            );
            ...
            for (const i of x) {
                for (const j of y) {
                    for (const k of z) {
                        ...
                        indicesVector
                            .applyAxisAngle(axis, (dir * Math.PI) / 2)
                            .round();
    ...
    doMove(center, dir, axis, speed) {
        return new Promise((resolve) => {
            ...
            const tween = new Tween(startRotation)
                .to(finalRotation, moveTotalTime)
                .easing(Easing.Sinusoidal.InOut)
                .onUpdate(function ({ theta }) {
                    // animating the rotation
                    center.rotateOnWorldAxis(
                        axis,
                        theta - previousRotation.theta
                    );
